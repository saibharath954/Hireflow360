...
export default function CandidateDetail() {
  ...
  const { getCandidate, generateMessagePreview, sendMessage, simulateReply, approveAndSendMessage, reprocessResume, error } = useApi();
  ...
  const pendingHRReviews = useMemo(() => {
    if (!candidate) return [];
    return candidate.messages.filter(
  ...
  const handleApproveAndSend = async (messageId: string, content: string) => {
    if (!id) return;
    setIsApproving(true);
    const result = await approveAndSendMessage(id, messageId, content);
    if (result) {
  ...
  const handleReprocessResume = async (resumeId: string) => {
    ...
    if (result) {
      ...
      toast.error("Reprocess failed", "Failed to reprocess the resume. Please try again.");
    }
  ...
  if (error || !candidate) {
    return <ErrorState message={error || "Candidate not found"} onRetry={loadCandidate} />;
  }
              ...
              {preview && (
                <div className="p-3 bg-muted rounded-lg">
                  ...
                  {previewAskedFields.length > 0 && (
                    <p className="text-xs text-muted-foreground mt-2">
                      Will ask about: {previewAskedFields.join(", ")}
                    </p>
                  )}
